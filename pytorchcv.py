...
def load_fashion_mnist(batch_size=64): # load_fashion_mnist라는 이름의 함수를 정의하고, 이 함수는 기본적으로 batch_size 매개변수를 64로 설정합니다. 이 매개변수는 데이터를 얼마나 많은 단위로 나눌지 결정
    ...
    builtins.test_loader = torch.utils.data.DataLoader(data_test, batch_size=batch_size, shuffle=False)

# 데이터셋의 샘플 이미지를 시각화하여 탐색함
def explore_dataset():
    display_dataset(data_train, n=10, classes=[
